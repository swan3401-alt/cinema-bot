...
export async function POST(req: NextRequest) {
  try {
    const { seatIds, sessionId, telegramId, locale } = await req.json();

    if (!seatIds?.length || !sessionId || !telegramId) {
    ...
    const bookings = await prisma.$transaction(
      seatIds.map((seatId: string) =>
        prisma.booking.create({
          data: { sessionId, seatId, telegramId: String(telegramId), locale: locale ?? "uz", token: nanoid(12), status: "PENDING" },
        })
      )
    );
